...
# Capture, then match with a here-string: `git config … | grep -q` lets grep close
# the pipe on its first match, git takes SIGPIPE (141), and under a hook's `pipefail`
# the pipeline reads as failed even when the include WAS present.
_git_include_present() {
  local paths; paths="$(git config --global --get-all include.path 2>/dev/null)"
  grep -qxF "$(_git_fragment)" <<<"$paths"
}
...
# No-op backup/restore. RFC-0004 §4.10 item 8: a module with no persistent state of
# its own answers the platform verbs rather than staying silent. core/git owns exactly
# one file — the managed fragment — and `install`/`update` regenerate it verbatim from
# the repo. There is nothing here that a backup could preserve and a reinstall could
# not. The user's own ~/.gitconfig is theirs, not Atlas's, and is never archived.
module::backup() {
  log::info "nothing to back up: core/git's only owned file is regenerated by 'atlas install'"
  return 0
}

module::restore() {
  log::info "nothing to restore: run 'atlas install core/git' to regenerate the managed fragment"
  return 0
}
...
module::verify() {
  ...
  # precedence order, so ours appears even when the user overrides it. Capture then
  # match (here-string) — a `| grep -q` would SIGPIPE git and misread under pipefail.
  local _resolved; _resolved="$(git config --global --includes --get-all init.defaultBranch 2>/dev/null)"
  grep -qxF "main" <<<"$_resolved" \
